...
sys.path.append('/home/emanuele/chemical_software/proxima/PyProxima/')
# sys.path.append('/home/emanuele/Postdoc_Pisa/results/force_field/SmartField/smart_dihedrals')
...
def main():
    ...
    for i, x in enumerate(tors_mean_list):
        ...
        # last= atoms_indices[i][-1]

        for k in range(0, new_mol.getNumAtoms()):
            at_tmp = new_mol.getAtomFromIndex(k)
            if at_tmp.getNumBonds(0) == 1:
                last = at_tmp.getSerial()
                print(last)
                break
    
        # file_out_qm = str(i) + '_qm.gjf'
        # string_qm_chk = '%chk=' + str(i) + '_qm.chk'
        file_out_xyz = str(i) + '.xyz'
        # title = 'D: ' + str(x)

        print("file {}".format(file_out_xyz))


        # out.toMol2(new_mol, 0, file_out_mm.encode('utf-8'))
        out.toMultimodelXYZ(new_mol, file_out_xyz.encode('utf-8'))
        ...
        #                  b'#p opt=modredundant  nosymm B3LYP/def2TZVP'], title.encode('utf-8') )

        # mapOfIndeces = out.toCOM_withTorsion(new_mol, 0, file_out_mm.encode('utf-8'), \
        #                 [b'%mem=1GB', \
        #                  b'%nprocshared=1', \
        #                  string_mm_chk.encode('utf-8'), \
        #                  b'#p opt=Z-matrix  geom=nocrowd nosymm AMBER=HardFisrt'], title.encode('utf-8'), last)
        #                 #  b'#p opt=Z-matrix  geom=nocrowd nosymm AMBER=HardFisrt'], title.encode('utf-8'), tors_mean_list[i][2])

        # fromIndexToSerial = mapOfIndeces[0]
        # fromSerialToIndex = mapOfIndeces[1]

        # print("From ZMAT-Index to Serial: ", fromIndexToSerial )
        # print("From Serial to ZMAT-Index: ", fromSerialToIndex )

        # print()

        # print("indice di 10 ", fromSerialToIndex[9])
    ...
    for i, j in enumerate(tors_mean_list):
        fxyz = str(i) + '.xyz'
        print(f'{i} {fxyz}')
        fmm = str(i) + '_mm.gjf'
        fqm = str(i) + '_qm.gjf'
        ...
        mol = ob.OBMol()
        obConversion.ReadFile(mol, fxyz)
        obConversion.WriteFile(mol, fqm)
        # obConversion.WriteFile(mol, fmm)

        # # Call PyProxima 
        # parser = pyproxima.PyParser()
        # mol_xyz = parser.readXYZ(file_out_xyz.encode('utf-8'), b'test')
        # # print(i, mol_xyz.getNumAtoms())

        # # mol.computeBonds_Fast(0.2)
        # # old_serial = []

        # out = pyproxima.PyOutputter()
        # mol_xyz.computeTopology(0, 0.4)

        # out.toCOM_zmat(mol_xyz, 0, file_out_mm.encode('utf-8'), \
        #                 [b'%mem=1GB', \
        #                  b'%nprocshared=1', \
        #                  string_mm_chk.encode('utf-8'), \
        #                  b'#p opt=Z-matrix  geom=nocrowd nosymm AMBER=HardFisrt'], b'title')
        
        # out.toCOM_zmat(mol_xyz, 0, file_out_qm.encode('utf-8'), \
        #                 [b'%mem=1GB', \
        #                  b'%nprocshared=4', \
        #                  string_qm_chk.encode('utf-8'), \
        #                  b'#p opt=Z-matrix  geom=nocrowd nosymm B3LYP/def2SVP'], b'title')

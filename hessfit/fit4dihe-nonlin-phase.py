...
def torsion_matrix_with_phase(xval_deg):
    """
    Fourier basis allowing phase fitting while remaining linear
    """
    x = np.deg2rad(xval_deg)

    A = np.column_stack([
        np.ones_like(x),          # constant offset
        np.cos(x),
        np.sin(x),
        np.cos(2*x),
        np.sin(2*x),
...
def weighted_error(coeffs, A, target, T):
    E_model = A @ coeffs
    residual = E_model - target
    
    if T == 0:
        w = np.ones_like(target)
    else:
        w = boltzmann_weights(target, T)
    
    return np.sqrt(np.sum((residual ** 2) * w) / len(target))
...
def convert_to_amber(coeffs, atomtypes="c -n -c3-os"):
    """
    Convert cos/sin Fourier coefficients to AMBER form
    """
    ...
    for n in range(1,4):
        ...
        print(f"{atomtypes:15s}  1  {2*k:8.4f}  {phase:6.2f}  {n}")
